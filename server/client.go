...
func (c *client) processInboundClientMsg(msg []byte) (bool, bool) {
	...
	if !ok {
		...
		r, _ = acc.sl.Match(string(c.pa.subject))
		if len(r.psubs)+len(r.qsubs) > 0 {
			if existing, ok := c.in.results[string(c.pa.subject)]; ok {
				existing.returnToPool()
			}
			// Prune the results cache. Keeps us from unbounded growth. Random delete.
			if len(c.in.results) >= maxResultCacheSize {
				n := 0
				for subject, r := range c.in.results {
					r.returnToPool()
...
func (c *client) handleGWReplyMap(msg []byte) bool {
	...
	if c.srv.gwLeafSubs.Count() > 0 {
		r, rc := c.srv.gwLeafSubs.MatchBytes(c.pa.subject)
		if len(r.psubs) > 0 {
...
func (c *client) getAccAndResultFromCache() (*Account, *SublistResult) {
	...
	if !ok {
		...
		r, _ = sl.MatchBytes(c.pa.subject)
