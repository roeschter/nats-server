...
const (
	...
	// JSStreamExpectedLastSequencePerSubjectConflict subject for expected last sequence is in process
	JSStreamExpectedLastSequencePerSubjectConflict ErrorIdentifier = 10160

	// JSStreamExpectedLastSequencePerSubjectNotReady expected last sequence per subject temporarily unavailable
	JSStreamExpectedLastSequencePerSubjectNotReady ErrorIdentifier = 10159
...
var (
	ApiErrors = map[ErrorIdentifier]*ApiError{
		...
		JSStreamExpectedLastSequencePerSubjectConflict: {Code: 409, ErrCode: 10160, Description: "subject for expected last sequence is in process"},
		JSStreamExpectedLastSequencePerSubjectNotReady: {Code: 503, ErrCode: 10159, Description: "expected last sequence per subject temporarily unavailable"},
...
// NewJSStreamExpectedLastSequencePerSubjectConflictError creates a new JSStreamExpectedLastSequencePerSubjectConflict error: "subject for expected last sequence is in process"
func NewJSStreamExpectedLastSequencePerSubjectConflictError(opts ...ErrorOption) *ApiError {
	eopts := parseOpts(opts)
	if ae, ok := eopts.err.(*ApiError); ok {
		return ae
	}

	return ApiErrors[JSStreamExpectedLastSequencePerSubjectConflict]
}

// NewJSStreamExpectedLastSequencePerSubjectNotReadyError creates a new JSStreamExpectedLastSequencePerSubjectNotReady error: "expected last sequence per subject temporarily unavailable"
func NewJSStreamExpectedLastSequencePerSubjectNotReadyError(opts ...ErrorOption) *ApiError {
	eopts := parseOpts(opts)
	if ae, ok := eopts.err.(*ApiError); ok {
		return ae
	}

	return ApiErrors[JSStreamExpectedLastSequencePerSubjectNotReady]
}
